...
int gameStage_16() {
	...
	if (Stage16_story_index == 0) {
		...
		Text(5, 7, "주인공을 발견하자마자 환하게 웃으며 걸어온다.");

		Text(5, 9, "\"민우 씨! 정각에 딱 맞췄네요?");
		Text(5, 10, "근데 오늘 주말이라 그런지 공원에 사람 진짜 바글바글하다…\"");

		Text(5, 12, "\"우리 어디로 갈까요? 민우 씨가 리드해 줘요!\"");
		...
		if (choice == 1) {
			...
			Text(5, 5, "주인공이 자신 있게 앞장서서 인파 속으로 뛰어들었다.");

			Text(5, 7, "하지만 사방에서 터지는 노점상 음악 소리와 수많은 인파에 이리저리 치이기 시작했다.");

			Text(5, 9, "민지는 점점 말수가 적어지더니 안색이 창백해졌다.");

			Text(5, 11, "\"민우 씨... 저 귀가 너무 먹먹하고 어지러워서…\"");
			Text(5, 12, "어제 먹은 치킨이 체했나 봐요. 오늘은 그냥 먼저 들어갈게요. 미안해요...\"");

			Text(5, 14, "-띠링");

			Text(5, 16, "[히로인 에너지 고갈 / 데이트 조기 종료]");
			Text(5, 17, "[피로도 과다로 인한 루트 이탈.]");
		...
		if (choice == 2) {
			...
			Text(5, 5, "주인공이 제안하자 민지는 조금 곤란한 표정을 지었다.");

			Text(5, 7, "\"아, 카페요? 안 그래도 아까 나오면서 집에서 커피를 큰 걸로 마시고 나와서…\"");

			Text(5, 9, "\"지금은 또 물 마시기가 조금 힘든데 어쩌죠? 날씨도 좋은데 조금 걷고 싶기도 하구…\"");

			Text(5, 11, "민지는 은근히 야외 산책을 원하고 있었고, 두 사람 사이의 대화 흐름이 삐끗하며 서먹해졌다.");

			Text(5, 13, "-띠링");

			Text(5, 15, "[히로인 요구 파악 실패 / 대화 불통]");
			Text(5, 16, "[데이트 템포 조절 실패로 관계 정체.]");
		...
		if (choice == 3) {
			...
			Text(5, 3, "\"저기 분수대 뒤쪽으로 돌아서 들어가면 사람들이 잘 모르는 작은 숲길 산책로가 있어요.\"");

			Text(5, 5, "\"거긴 한적하니까 우리 조용히 걸어요.\"");

			Text(5, 7, "민지의 커다란 눈이 동그랗게 커지더니 이내 안도의 미소를 짓는다.");

			Text(5, 9, "\"와, 진짜요? 안 그래도 사람 많은 데 오면 금방 지치는 편이라 걱정했는데…\"");

			Text(5, 11, "\"민우 씨 센스 대박! 어제도 그렇고 진짜 눈치 빠르시네요. 얼른 가요, 얼른!\"");

			Text(5, 13, "조용한 숲길로 들어서자 민지가 기분이 좋아졌는지");
			Text(5, 14, "가벼운 발걸음으로 주인공의 곁에 바짝 붙어 걸어온다.");
...
int Stage17_story_index = 0;

int gameStage_17() {

	if (Stage17_story_index == 0) {
		Text(5, 3, "풀냄새가 싱그럽게 퍼지는 숲길을 나란히 걷던 중");
		Text(5, 4, "민지가 문득 걸음을 늦추며 주인공을 힐끔 쳐다보았다.");

		Text(5, 6, "\"근데 민우 씨. 어제 카페에서 번호 교환하고 밤에 치맥 먹고,");
		Text(5, 7, "오늘이 겨우 이틀째잖아요.\"");

		Text(5, 9, "\"솔직히 저에 대해 아는 것도 별로 없으실 텐데…");
		Text(5, 10, "갑자기 오늘 보자고 한 진짜 이유가 뭐예요?\"");

		Text(5, 12, "\"아까 톡으로 할 말도 있다고 하셔서 궁금했어요.\"");

		Text(5, 14, "민지는 가벼운 농담보다는 대화의 진정성을 확인하고 싶어 하는 눈치다.");

		move_cursor(146, 8);
		printf("당신의 대답을 고르세요");

		move_cursor(139, 12);
		printf("1. 어제 치맥을 너무 맛있게 먹어서...");

		move_cursor(139, 17);
		printf("2. 그냥 주말인데 마침 심심...");

		move_cursor(139, 22);
		printf("3. 민지 씨 생각이 계속 나서...");
		...
		if (choice == 1) {
			...
			Text(5, 3, "\"어제 치맥을 너무 맛있게 먹어서 그런지,");
			Text(5, 4, "오늘도 맛있는 거 같이 먹으러 가자고 하려고 불렀어요.\"");

			Text(5, 6, "주인공이 멋쩍게 웃으며 말했으나, 민지는 조금 아쉬운 기색을 보였다.");

			Text(5, 8, "\"아…… 먹는 것 때문에 부르신 거구나ㅋㅋㅋ\"");
			Text(5, 9, "\"전 또 뭐라고요. 전 나름 어제 재밌었어서 기대하고 나왔는데,");
			Text(5, 10, "그냥 밥 친구가 필요하셨나 봐요.\"");

			Text(5, 12, "민지가 장난치듯 넘겼으나,");
			Text(5, 13, "은근히 실망한 분위기가 흐르며 대화가 소원해졌다.");

			Text(5, 15, "-띠링");

			Text(5, 17, "[이성으로서의 긴장감 완화 / 밥 친구 판정]");
			Text(5, 18, "[데이트 분위기 반전 실패.]");
		...
		if (choice == 2) {
			...
			Text(5, 3, "\"그냥 주말인데 마침 심심하기도 하고,");
			Text(5, 4, "바로 옆집 살아서 편하게 볼 수 있으니까 불렀죠.\"");

			Text(5, 6, "주인공이 덤덤하게 말하자,");
			Text(5, 7, "민지의 표정이 조금 굳어졌다.");

			Text(5, 9, "\"아…… 심심해서 부르신 거구나.\"");

			Text(5, 11, "\"전 어제 민우 씨가 되게 다정하게 챙겨주셔서");
			Text(5, 12, "진지하게 친해지고 싶어 나온 건데,");
			Text(5, 13, "그냥 시간 때우기용 이웃이었나 봐요.\"");

			Text(5, 15, "\"조금 실망이네요.\"");

			Text(5, 17, "민지는 약속이 생각났다며 슬그머니 자리를 피했고,");
			Text(5, 18, "어제 쌓은 호감도가 전부 깎여 나갔다.");

			Text(5, 20, "-띠링");

			Text(5, 22, "[성의 없는 답변으로 인한 호감도 하락]");
			Text(5, 23, "[히로인의 흥미 상실.]");
		...
		if (choice == 3) {
			...
			Text(5, 3, "주인공은 걸음을 멈추고 민지의 눈을 정면으로 응시하며 진지하게 말했다.");

			Text(5, 5, "\"옆집 이웃이라서 부른 게 아니에요.");
			Text(5, 6, "그냥 어제 같이 치맥 먹고 헤어진 순간부터,");
			Text(5, 7, "머릿속에서 민지 씨 생각이 계속 떠나지 않더라고요.\"");

			Text(5, 9, "\"솔직하게 말씀드리고 싶었어요.\"");

			Text(5, 11, "순간 민지의 양 볼이 붉어지기 시작했다.");

			Text(5, 13, "그녀는 부끄러운지 고개를 살짝 숙이고 숲길 바닥을 보다가,");
			Text(5, 14, "배시시 미소를 지으며 주인공을 올려다보았다.");

			Text(5, 16, "\"……그렇게 직구로 말하면 부끄럽잖아요.\"");

			Text(5, 18, "\"근데 솔직하게 말해줘서 고마워요.");
			Text(5, 19, "사실 저도 오늘 민우 씨 만나러 나오면서 조금 설렜거든요.\"");

			Text(5, 21, "두 사람 사이에 흐르는 공기가 완벽하게 연인의 분위기로 뒤바뀐다.");
...
int Stage18_story_index = 0;

int gameStage_18() {

	if (Stage18_story_index == 0) {
		Text(5, 3, "어느새 산책로의 끝자락, 주홍빛 노을이 아름답게 깔리는 공원 갈림길에 다다랐다.");

		Text(5, 5, "민지가 아쉬움이 가득 찬 눈빛으로 천천히 돌아서며 주인공을 바라보았다.");

		Text(5, 7, "\"시간 진짜 빠르다…… 민우 씨랑 있으면 한 시간이 일 분 같아요.\"");

		Text(5, 9, "\"저 이제 슬슬 집에 가봐야 할 것 같아요……\"");

		Text(5, 11, "지금이다. 무한 루프의 세계선을 찢고 진심을 던질 유일무이한 타이밍.");
		...
		if (choice == 1) {
			...
			Text(5, 3, "\"시간도 늦었는데 조심히 들어가세요!\"");

			Text(5, 5, "마지막 순간에 긴장한 나머지 고백 한마디 던지지 못하고 굳어버렸다.");
			Text(5, 6, "어설프게 배웅 인사를 건넸다.");

			Text(5, 8, "민지는 고백을 기대했던 것인지,");
			Text(5, 9, "허무하고 실망스러운 표정으로 씁쓸하게 뒤돌아섰다.");

			Text(5, 11, "잠시 후 도착한 카톡:");

			Text(5, 13, "'민우 씨, 좋은 분 같지만 생각해보니까");
			Text(5, 14, "계속 단둘이 만나는 건 좀 부담스러운 것 같아서요..");
			Text(5, 15, "당분간은 조용히 지내는 게 좋을 것 같아요.'");

			Text(5, 17, "망설임의 대가는 혹독했다. 다가갈 명분이 소멸했다.");

			Text(5, 19, "자정 정각.");

			Text(5, 21, "-띠링");

			Text(5, 23, "[용기 부족으로 인한 타임 오버 / 히로인 마음 정리]");
		...
		if (choice == 2) {
			...
			Text(5, 3, "\"우리 어차피 바로 옆집이잖아요.\"");

			Text(5, 5, "\"이대로 헤어지긴 아쉬우니까 조금만 더 같이 산책하다가 들어가면 안 돼요?\"");

			Text(5, 7, "고백 대신 시간만 끌려는 태도에 민지가 조금 의아한 표정을 지었다.");

			Text(5, 9, "\"더 같이 있는 건 좋은데…\"");
			Text(5, 10, "\"아까 톡으로 할 말 있다는 건 언제 해 주실 거예요?\"");
			Text(5, 11, "\"자꾸 말을 돌리시는 것 같아서요.\"");

			Text(5, 13, "진정성 없이 관계를 유예하려는 모습에 민지는 피곤함을 느꼈고");

			Text(5, 15, "결국 타이밍을 놓쳐 흐지부지하게 헤어졌다.");

			Text(5, 17, "-띠링");

			Text(5, 19, "[핵심을 피하는 유약함 판정]");
			Text(5, 20, "[진전 없는 관계 고착화.]");
		...
		if (choice == 3) {
			...
			Text(5, 3, "용기를 내어 민지의 작고 부드러운 두 손을 조심스럽게 맞잡았다.");

			Text(5, 5, "민지가 놀란 듯 숨을 흡 들이쉬며 주인공을 바라본다.");

			Text(5, 7, "주인공은 그녀의 맑은 눈동자를 똑바로 쳐다보며 진심을 담아 말했다.");

			Text(5, 9, "\"만난 지는 얼마 안 됐지만, 제 진심은 그 누구보다 깊습니다.\"");

			Text(5, 10, "\"민지 씨랑 그냥 옆집 이웃이 아니라,");
			Text(5, 11, "매일 아침과 밤을 함께하는 연인이 되고 싶어요.\"");

			Text(5, 13, "\"저랑 사귀어주세요.\"");

			Text(5, 15, "민지는 얼굴이 붉어진 채 굳어버렸다.");

			Text(5, 17, "짧은 정적이 흐르고, 주인공의 심장이 터지기 직전—");

			Text(5, 19, "이윽고 민지가 수줍게 웃으며");
			Text(5, 20, "주인공의 품으로 한 걸음 가까이 다가왔다.");

			Text(5, 22, "\"……되게 훅 들어오시네.\"");

			Text(5, 24, "\"사실 저도 어제 민우 씨가");
			Text(5, 25, "매너 있게 챙겨줄 때부터 자꾸 생각나고 신경 쓰였거든요.\"");

			Text(5, 27, "\"좋아요…… 우리 예쁘게 만나봐요, 민우 씨.\"");

			Text(5, 29, "그녀가 고개를 들며 세상에서 가장 환한 미소를 지어 보였다.");

			Text(5, 31, "-띠링");

			Text(5, 33, "[★ QUEST CLEAR ★]");
			Text(5, 34, "[히로인 '김민지' 공략 완벽 성공!]");
...
int Ending_HAPPY() {
	...
	Text(5, 5, "주인공은 번쩍 눈을 떴다.");

	Text(5, 7, "익숙한 천장이 보였지만, 무언가 확실하게 달랐다.");

	Text(5, 9, "항상 눈을 뜨자마자 시야를 가리던 시스템 메시지 창이 나타나지 않았다.");

	Text(5, 11, "불안한 마음으로 서둘러 스마트폰을 켜 화면 상단을 확인했다.");

	Text(5, 13, "[6월 4일 목요일]");

	Text(5, 15, "매일 아침 '6월 3일 수요일'에 멈춰있던 시간이 마침내 흘러가기 시작했다.");
	Text(5, 16, "지독하게 반복되던 어제가 끝나고, 마침내 세계에 '내일'이 찾아온 것이다.");

	Text(5, 18, "-카톡!");

	Text(5, 20, "타이밍 좋게 알림음이 울렸다.");

	Text(5, 22, "프로필 사진 속 환하게 웃고 있는 그녀의 이름 옆에 어제까진 없었던 하트(♥)가 예쁘게 붙어있었다.");

	Text(5, 24, "김민지♥: \"민우 씨 좋은 아침! 우리 사귀고 맞는 첫날 아침이네용ㅎㅎ\"");
	Text(5, 25, "김민지♥: \"출근할 때 복도에서 같이 출발해요! 문 앞에서 기다릴게요!\"");

	Text(5, 27, "주인공은 침대에서 일어나 현관문으로 달려갔다.");

	Text(5, 29, "문을 열자, 어제 노을빛 아래서 나를 보고 웃어주던 가장 사랑스러운 이웃이자 여자친구가 서 있었다.");

	Text(5, 31, "\"안녕, 민우 씨. 오늘부터 잘 부탁해요!\"");

	Text(5, 33, "지독했던 타임 루프의 끝에는");
	Text(5, 34, "그 어떤 반복되는 시간보다 찬란한 그녀와의 매일이 기다리고 있었다.");

	Text(5, 36, "[TRUE ENDING : 《30살까지 모솔이면 마법사가 된다는 소문이 사실인 것 같습니다》 - 완]");
