...
int gameStage_prolog() {
	...
	if (prolog_story_index == 1) {
		Sleep(2000);
		move_cursor(5, 3);
		printf("눈을 뜬 당신. 원룸 천장.");

		Sleep(2000);
		move_cursor(5, 4);
		printf("널브러진 컵라면, 먼지 쌓인 키보드, 모니터 속 정지된 미연시 게임 화면.");
		...
		Sleep(2000);
		move_cursor(5, 14);
		printf("컴퓨터 화면이 지직거리며 정체불명의 문장이 떠오른다.");

		Sleep(2000);
		move_cursor(5, 16);
		printf("「30년간 연애 경험이 없는 자에게 자격을 부여합니다.」");
		...
		Sleep(2000);
		move_cursor(5, 23);
		printf("「공략 실패시. 하루를 재시작합니다.」");

		Sleep(2000);
		move_cursor(5, 24);
		printf("「고백하지 못하면, 내일도 오지 않는다.」");
		...
		Sleep(2000);
		move_cursor(5, 28);
		printf("《아무래도 30살까지 모솔이면 마법사가 되는 것 같습니다》");
		Sleep(4000);
	...
	if (prolog_story_index == 2) {
		move_cursor(5, 3);
		printf(" 프롤로그 종료. 오른쪽 방향키를 누르면 게임을 시작합니다.");
...
int gameStage_1() {
	...
	if (gameStage1_story_index == 0) {
		...
		move_cursor(5, 5);
		printf("[첫 번째 퀘스트가 도착했습니다.]");

		Sleep(2000);

		move_cursor(5, 7);
		printf("퀘스트 : 카페로 이동하시오.");


		Sleep(2000);

		move_cursor(5, 9);
		printf("[목적지에 도착하면 다음 퀘스트가 개방됩니다.]");
	...
	if (gameStage1_story_index == 2) {
		...
		if (choice == 2) {
			ClearStoryArea();
			move_cursor(5, 3);
			printf("정답입니다! 당신의 운명의 상대를 찾았습니다.");
			move_cursor(5, 10);
